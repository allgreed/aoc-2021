...
pub fn main() anyerror!void {
    ...
    for (lines.items) |line| {
        x_max = max3(x_max, line.start.x, line.end.x);
        y_max = max3(y_max, line.start.y, line.end.y);
    }
    ...
    for (lines.items) |line| {
        if (line.start.x == line.end.x) {
            ...
            std.log.info("{s}", .{line});
            var x: i64 = line.start.x;
            var y: i64 = line.start.y;

            const x_dir: i8 = if (line.end.x > line.start.x) 1 else -1;
            const y_dir: i8 = if (line.end.y > line.start.y) 1 else -1;

            while (x != line.end.x) {
                std.log.info("{d}x{d}", .{ x, y });
                map[@intCast(usize, x)][@intCast(usize, y)] += 1;
                x += x_dir;
                y += y_dir;
            }
